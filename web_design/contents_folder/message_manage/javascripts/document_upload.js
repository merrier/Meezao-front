...
$(document).ready(function(){
    ...
    var par=".document_upload";

    //------------------------------下拉菜单选择-------------------------------------------
    $(par+" .document_kind_dropdown").delegate(".dropdown-menu a","click",function(){
        var data_id=$(this).attr("data-id");
        var text=$(this).text();
        $(this).parents(".dropdown").attr("data-id",data_id).find("span").eq(0).text(text);
        $(this).parents(".dropdown").next(".document_kind").val(text);
    });
    //------------------------------下拉菜单选择-------------------------------------------
    ...
    $(par+" .charge_form").validate({
        ...
        submitHandler:function(form){
            console.log("文档分类"+$(".document_kind").val());
